...
const VendorProducts = () => {
  ...
  const fetchMenuItems = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.get('http://localhost:5000/api/food/vendor/menu', {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleMenuSubmit = async (e) => {
    ...
    try {
      ...
      if (editingItem) {
        await axios.put(
          `http://localhost:5000/api/food/menu/${editingItem._id}`,
          menuData,
          config
        );
        setSuccess('Menu item updated successfully!');
      } else {
        await axios.post('http://localhost:5000/api/food/menu', menuData, config);
  ...
  const handleDeleteMenuItem = async (menuItemId) => {
    ...
    try {
      const token = localStorage.getItem('token');
      await axios.delete(`http://localhost:5000/api/food/menu/${menuItemId}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      fetchMenuItems();
  ...
  return (
    <Container maxWidth="lg" sx={{ py: 4 }}>
      ...
      <Dialog open={openDialog && tabValue === 1} onClose={handleCloseDialog} maxWidth="md" fullWidth>
        ...
        <form onSubmit={handleMenuSubmit}>
          <DialogContent>
            ...
            <Grid container spacing={2}>
              <Grid item xs={12}>
                <TextField fullWidth label="Item Name" name="name" value={menuFormData.name} onChange={handleMenuChange} required />
              </Grid>
              <Grid item xs={12}>
                <TextField fullWidth label="Description" name="description" value={menuFormData.description} onChange={handleMenuChange} multiline rows={3} />
              </Grid>
              <Grid item xs={6}>
                <TextField fullWidth select label="Category" name="category" value={menuFormData.category} onChange={handleMenuChange} required>
                  {menuCategories.map((cat) => <MenuItem key={cat} value={cat}>{cat.charAt(0).toUpperCase() + cat.slice(1)}</MenuItem>)}
                </TextField>
              </Grid>
              <Grid item xs={6}>
                <TextField fullWidth label="Price" name="price" type="number" value={menuFormData.price} onChange={handleMenuChange} required inputProps={{ step: '0.01', min: '0' }} />
              </Grid>
              <Grid item xs={6}>
                <TextField fullWidth label="Preparation Time (minutes)" name="preparationTime" type="number" value={menuFormData.preparationTime} onChange={handleMenuChange} inputProps={{ min: '0' }} />
              </Grid>
              <Grid item xs={6}>
                <TextField fullWidth label="Allergens (comma separated)" name="allergens" value={menuFormData.allergens} onChange={handleMenuChange} placeholder="e.g., nuts, dairy, gluten" />
              </Grid>
              <Grid item xs={12}>
                <Typography variant="subtitle2" sx={{ mb: 1 }}>Dietary Options</Typography>
              ...
              <Grid item xs={12}>
                <Typography variant="subtitle2" sx={{ mb: 1 }}>Nutrition Information (optional)</Typography>
              </Grid>
